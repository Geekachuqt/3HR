...
struct GLTCH3HR : Module {
	...
	GLTCH3HR() {
		...
		configParam(END_CV_PARAM_PARAM, 0.f, 1.f, 0.f, "");
		configParam(START_CV_PARAM_PARAM, 0.f, 1.f, 0.f, "");
		configParam(END_PARAM_PARAM, 0.f, 1.f, 0.1f, "");
		configParam(START_PARAM_PARAM, 0.f, 1.f, 0.f, "");
		configParam(THRESHOLD_PARAM_PARAM, 0.f, 10.f, 0.f, "");
		configInput(AUDIO_IN_INPUT, "");
		configInput(END_CV_IN_INPUT, "");
		configInput(START_CV_IN_INPUT, "");
		configInput(THRESHOLD_IN_INPUT, "");
		configOutput(AUDIO_OUT_OUTPUT, "");
		configOutput(TEST_OUTPUT,"");
	}
	...
	void process(const ProcessArgs& args) override {
		...
		EndPoint = inputs[END_CV_IN_INPUT].isConnected() ? clamp((params[END_PARAM_PARAM].getValue()+params[END_CV_PARAM_PARAM].getValue()*inputs[END_CV_IN_INPUT].getVoltage()*0.1f),0.f,1.f)*args.sampleRate*0.5f : params[END_PARAM_PARAM].getValue()*args.sampleRate*0.5f;
		...
		switch(RecordingState){
			...
			case CROSSTART:
				...
				CrossCounter = 0;
				CrossStart = PlayBuffer[PlayHead];
			...
			case CROSSING:
				...
				Output = PlayBuffer[PlayHead]*(CrossCounter/CrossSamples)+((CrossSamples-CrossCounter)/CrossSamples)*CrossStart;
				PlayHead += Direction;
			...
			case PLAYING:
				...
				if(ThresholdInput < Threshold && RecordingState == PLAYING){
			...
			case ENDING:
				...
				PlayHead += clamp(Direction,-1.f,1.f);
			...
			case RESET:
				...
				PlayBuffer.resize(std::ceil(args.sampleRate*0.5f),0.f);
				PlayHead = 0;
				Output = 0;
				BufferCounter = 0;
				FadeOutCounter = 0;
...
struct GLTCH3HRWidget : ModuleWidget {
	GLTCH3HRWidget(GLTCH3HR* module) {
		...
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(15.237, 90.343)), module, GLTCH3HR::TEST_OUTPUT));
